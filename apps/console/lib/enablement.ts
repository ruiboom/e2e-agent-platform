...
export const STAGES: Stage[] = [
  ...
  { id: "specify", name: "Specify", phase: "Shape & plan", service: "router", route: "/projects/:slug/specify",
    ...
    howItWorks: "One model-router call turns a topic into three linked artifacts at once: a sectioned scope, a production-ready system prompt, and the knowledge-base topics to ground the agent. These feed Build and Ground.",
  ...
  { id: "architect", name: "Architect", phase: "Shape & plan", service: "router", route: "/projects/:slug/shape",
    ...
    howItWorks: "Capture the architecture decision record — build paradigm, runtime, retrieval strategy, storage projections, channels, deploy target, guardrail policy. Invalid combinations are rejected (e.g. graph retrieval without a graph projection). Build reads the ADR's retrieval strategy directly.",
    reads: "scope (+ constraints)", writes: "adr" },
  ...
  { id: "ground", name: "Ground", phase: "Make", service: "ground", route: "/projects/:slug",
    blurb: "The knowledge kernel: governed canonical store, six retrieval modes, pinned releases.",
    howItWorks: "Ingest content (docs, RSS, web, GitHub); each revision is scanned and SUBMITTED, then a different actor APPROVES it (four-eyes). Approved revisions are chunked, embedded (pgvector), and graph-enriched (Neo4j). A pinned release is what an agent consumes. Six retrieval modes: vector, lexical, hybrid, graph, graph_hybrid.",
  ...
  { id: "test", name: "Test", phase: "Prove", service: "eval", route: "/projects/:slug",
    blurb: "Generate a multi-persona test suite with coverage tags.",
    howItWorks: "From the agent's system prompt, generate personas + tagged test cases (topic / behaviour / scope-boundary / out-of-scope). Evaluate runs the agent against these, persona by persona.",
    reads: "agent_version", writes: "test_suite" },
  { id: "evaluate", name: "Evaluate", phase: "Prove", service: "eval", route: "/projects/:slug",
  ...
  { id: "operate", name: "Operate", phase: "Run & improve", service: "optimise", route: "/projects/:slug/chat",
    ...
    howItWorks: "Every chat turn is logged with its retrieval score. Operate diagnoses weak/off-topic turns and proposes an improved system prompt — emitted as a NEW system_prompt version that re-enters the pipeline (rebuild to adopt it). It's a proposal, never auto-promoted.",
